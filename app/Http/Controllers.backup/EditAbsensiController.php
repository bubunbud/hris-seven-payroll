...
class EditAbsensiController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        return view('edit-absensi.create', compact('karyawans', 'karyawanList'));
    }

    /**
     * Store new absensi data
     */
    public function store(Request $request)
    {
        ...
        $karyawan = Karyawan::where('Nik', $nikStr)
            ->where('vcAktif', '1')
            ->whereNull('Tgl_Berhenti')
            ->first();
        ...
        if ($absenExists) {
            return redirect()->route('edit-absensi.create')
                ->withInput()
                ->with('error', 'Data absensi untuk tanggal dan NIK tersebut sudah ada. Gunakan fitur Edit untuk mengubah data yang sudah ada.');
        }
        ...
        // Validasi: jam masuk dan keluar harus bersamaan atau kosong bersamaan
        if (($jamMasuk && !$jamKeluar) || (!$jamMasuk && $jamKeluar)) {
            return redirect()->route('edit-absensi.create')
                ->withInput()
                ->with('error', 'Jam masuk dan jam keluar harus diisi bersamaan atau dikosongkan bersamaan.');
        }

        // Keterangan untuk data baru
        $keteranganBaru = 'Input: ' . Carbon::now()->format('d/m H:i');
        $maxLength = 20;
        if (strlen($keteranganBaru) > $maxLength) {
            $keteranganBaru = substr($keteranganBaru, 0, $maxLength);
        }

        // Insert menggunakan DB::table karena composite primary key
        try {
