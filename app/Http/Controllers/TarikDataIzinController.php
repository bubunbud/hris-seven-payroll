...
class TarikDataIzinController extends Controller
{
    /**
     * Menampilkan form tarik data izin
     */
    public function index()
    {
        return view('tarik-data-izin.index');
    }

    /**
     * Proses tarik data izin dari server remote
     */
    public function pullData(Request $request)
    {
        ...
        try {
            ...
            if ($remoteData->isEmpty()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Tidak ada data izin ditemukan untuk periode yang dipilih.'
                ], 404);
            }

            // Proses insert data ke database lokal (skip jika sudah ada)
            $inserted = 0;
            $skipped = 0;
            $errors = [];
            ...
            foreach ($remoteData as $remoteRow) {
                try {
                    // Cek apakah data sudah ada (berdasarkan primary key: vcCounter)
                    $vcCounter = $remoteRow->vcCounter ?? null;
                    
                    if (!$vcCounter) {
                        // Jika vcCounter tidak ada di data remote, generate baru
                        $vcCounter = $this->generateVcCounter();
                    }

                    $existing = DB::table('t_izin')
                        ->where('vcCounter', $vcCounter)
                        ->first();
                    ...
                    foreach ($fields as $field) {
                        if ($field === 'vcCounter' && !isset($remoteRow->vcCounter)) {
                            // Jika vcCounter tidak ada di remote, gunakan yang sudah di-generate
                            $data[$field] = $vcCounter;
                        } else {
                            $data[$field] = $remoteRow->$field ?? null;
                        }
                    }

                    // Pastikan vcCounter selalu ada
                    if (!isset($data['vcCounter'])) {
                        $data['vcCounter'] = $vcCounter;
                    }

                    if ($existing) {
                        // Data sudah ada di lokal, SKIP (tidak copy/update)
                        // Logika: Hanya copy data yang belum ada, skip data yang sudah ada
                        $skipped++;
                    } else {
                        // Data belum ada, insert data baru
                        $data['dtCreate'] = Carbon::now();
                        $data['dtChange'] = Carbon::now();

                        DB::table('t_izin')->insert($data);
                        $inserted++;
                    }
                } catch (\Exception $e) {
                    $errors[] = [
                        'vcCounter' => $remoteRow->vcCounter ?? 'N/A',
                        'nik' => $remoteRow->vcNik ?? 'N/A',
                        'tanggal' => $remoteRow->dtTanggal ?? 'N/A',
                        'error' => $e->getMessage()
                    ];
                    Log::error('Error inserting izin data', [
                        'vcCounter' => $remoteRow->vcCounter ?? null,
            ...
            $message = "Data berhasil ditarik! Insert: {$inserted}, Skip: {$skipped}";
            if (!empty($errors)) {
            ...
            DB::rollBack();
            Log::error('Error pulling izin data', [
                'error' => $e->getMessage(),
    ...
    /**
     * Generate vcCounter unik (panjang 9)
     */
    private function generateVcCounter()
    {
        $vcCounter = null;
        for ($i = 0; $i < 5; $i++) {
            $candidate = substr(
            ...
            if (!DB::table('t_izin')->where('vcCounter', $candidate)->exists()) {
                $vcCounter = $candidate;
                break;
            }
        ...
        // Jika masih null, gunakan timestamp + random
        if (!$vcCounter) {
            $vcCounter = substr(str_replace(['-', ':', ' '], '', Carbon::now()->toDateTimeString()) . mt_rand(100, 999), 0, 9);
        }
