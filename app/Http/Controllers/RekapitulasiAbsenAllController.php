...
class RekapitulasiAbsenAllController extends Controller
{
    ...
    private function calculateRekapitulasiKaryawan($karyawan, $startDate, $endDate, $jumlahHariKerja, $no)
    {
        ...
        // Hitung per jenis tidak masuk (hanya hari kerja normal, dengan mempertimbangkan tukar hari kerja)
        $sakit = 0;              // S010
        $izinPribadi = 0;        // I002
        $izinResmi = 0;          // I001
        ...
        foreach ($tidakMasuk as $tm) {
            ...
            // Hitung hanya hari kerja normal dalam range tidak masuk (dengan mempertimbangkan tukar hari kerja)
            $hariKerjaNormal = 0;
            $cursor = $mulai->copy();
            while ($cursor->lte($selesai)) {
                $tanggalStr = $cursor->format('Y-m-d');
                
                // Hanya hitung jika hari kerja normal (dengan mempertimbangkan tukar hari kerja) dan dalam range periode
                if ($this->isHariKerjaNormal($tanggalStr, $nik) && 
        ...
        // Filter $tanggalHadirList untuk hanya hari kerja normal (dengan mempertimbangkan tukar hari kerja)
        $tanggalHadir = [];
        foreach ($tanggalHadirList as $tanggalStr) {
            // Hanya masukkan jika hari kerja normal (dengan mempertimbangkan tukar hari kerja)
            if ($this->isHariKerjaNormal($tanggalStr, $nik)) {
        ...
        // Re-calculate Hadir (H) berdasarkan formula: H = JHK - (S + I + A + IR + IO + CT + CM)
        // Ini memastikan H mencakup semua hari kerja normal, termasuk yang ditukar menjadi hari kerja
        $totalTidakMasuk = $sakit + $izinPribadi + $alfa + $izinResmi + $izinOrganisasi + $cutiTahunan + $cutiMelahirkan;
        $hadir = max(0, $jumlahHariKerja - $totalTidakMasuk);
        ...
        foreach ($izinKeluarPribadi as $izin) {
            ...
            // Hanya hitung jika hari kerja normal (dengan mempertimbangkan tukar hari kerja)
            if ($this->isHariKerjaNormal($tanggalIzin, $nik)) {
    ...
    private function calculateRekapitulasiKaryawanOptimized($karyawan, $startDate, $endDate, $jumlahHariKerja, $no, $holidayDates, $absenList, $tidakMasukList, $izinList)
    {
        ...
        // 1. Hadir (H) - hanya hari kerja normal (dengan mempertimbangkan tukar hari kerja)
        ...
        // 5. Terlambat (T) - jam masuk > jam masuk shift-nya (hanya hari kerja normal, dengan mempertimbangkan tukar hari kerja)
        $terlambat = 0;
        foreach ($absenList as $ab) {
            $tanggalStr = $ab->dtTanggal instanceof Carbon 
                ? $ab->dtTanggal->format('Y-m-d') 
