...
class RekapUpahKaryawanController extends Controller
{
    /**
     * Display form untuk cetak rekap upah karyawan
     */
    public function index()
    {
        ...
        return view('laporan.rekap-upah-karyawan.index', compact('divisis', 'defaultPeriode'));
    }

    /**
     * Preview/Print rekap upah karyawan berdasarkan filter
     */
    public function preview(Request $request)
    {
        ...
        if ($closings->isEmpty()) {
            return redirect()->route('rekap-upah-karyawan.index')
        ...
        return view('laporan.rekap-upah-karyawan.preview', compact(
            'groupedData',
            'grandTotal',
            'tanggalAwal',
            'tanggalAkhir',
    ...
    private function calculateTotal($closings)
    {
        ...
        foreach ($closings as $closing) {
            ...
            $total['tsm'] += 0; // TSM selalu 0 berdasarkan contoh
        ...
        // Calculate persen lembur (rata-rata persentase, bukan total)
        if ($total['gaji'] > 0) {
            $total['persen_lembur'] = ($total['upah_lembur'] / $total['gaji']) * 100;
        }
