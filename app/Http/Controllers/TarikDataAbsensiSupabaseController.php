...
use App\Services\SupabaseAttendanceService;
use Carbon\Carbon;
...
class TarikDataAbsensiSupabaseController extends Controller
{
    ...
    {
        return view('tarik-data-absensi-supabase.index', [
            'apiBaseUrl' => $this->supabaseService->getBaseRestUrl(),
    ...
    /**
     * Tarik log dari Supabase → preview (dry run), belum simpan ke t_absen.
     */
    public function pull(Request $request)
    {
        ...
        $result = $this->supabaseService->pullLogs($startDate, $endDate, $nikFilter);
        ...
        $rawLogs = $result['data'] ?? [];
        $aggregated = $this->supabaseService->aggregateLogs($rawLogs);
        ...
        $meta = $result['meta'] ?? [];
        $apiRawTotal = (int) ($meta['total_raw'] ?? count($rawLogs));
        $skippedNoNik = (int) ($meta['skipped_no_nik'] ?? 0);

        $message = 'Berhasil menarik ' . count($rawLogs) . ' log dari Supabase (raw API: ' . $apiRawTotal . ' baris).';
        if ($apiRawTotal === 0) {
            $message = 'Koneksi Supabase berhasil, tetapi tidak ada data di tabel '
                . ($meta['table'] ?? 'attendance')
                . ' untuk periode ' . $startDate . ' s/d ' . $endDate
                . '. Pastikan data sudah masuk ke Supabase atau gunakan service role key jika RLS aktif.';
        } elseif ($skippedNoNik > 0) {
            $message .= ' ' . $skippedNoNik . ' baris dilewati karena NIK tidak ditemukan (cek tabel profiles).';
        }
    ...
    public function save(Request $request)
    {
        ...
        try {
            foreach ($aggregated as $row) {
                ...
                $hasJam = !$this->supabaseService->isEmptyTime($jamMasuk) || !$this->supabaseService->isEmptyTime($jamPulang);
                if (!$hasJam) {
                ...
                try {
                    ...
                    if ($existing) {
                        ...
                        if ($this->supabaseService->isEmptyTime($dbJamMasuk) && !$this->supabaseService->isEmptyTime($jamMasuk)) {
                            $updatePayload['dtJamMasuk'] = $jamMasuk;
                        }
                        if ($this->supabaseService->isEmptyTime($dbJamKeluar) && !$this->supabaseService->isEmptyTime($jamPulang)) {
                    ...
                    Log::error('Error saving Supabase attendance', [
                        'nik' => $nik,
                        'tanggal' => $tanggal,
                        'error' => $e->getMessage(),
                    ]);
            ...
            $message = "Data Supabase berhasil disimpan. Insert: {$inserted}, Update: {$updated}";
            if ($skipped > 0) {
            ...
            DB::rollBack();
            Log::error('Error saving Supabase batch', ['error' => $e->getMessage()]);
    ...
    protected function buildSavePreview(array $aggregated): array
    {
        ...
        foreach ($aggregated as $row) {
            ...
            } elseif ($this->supabaseService->isEmptyTime($row['jam_masuk'] ?? null) && $this->supabaseService->isEmptyTime($row['jam_pulang'] ?? null)) {
                $aksi = 'Lewati';
                $keterangan = 'Tidak ada jam masuk/pulang';
            } elseif ($existing) {
                $canUpdateMasuk = $this->supabaseService->isEmptyTime($existing->dtJamMasuk ?? null) && !$this->supabaseService->isEmptyTime($row['jam_masuk'] ?? null);
                $canUpdatePulang = $this->supabaseService->isEmptyTime($existing->dtJamKeluar ?? null) && !$this->supabaseService->isEmptyTime($row['jam_pulang'] ?? null);
