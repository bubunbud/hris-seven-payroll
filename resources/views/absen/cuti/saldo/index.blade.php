        ...
        <div class="col-12">
            ...
            <div class="card mb-3">
                <div class="card-header bg-success text-white">
                    <h5 class="mb-0"><i class="fas fa-file-excel me-2"></i>Import/Update Saldo Cuti dari Excel</h5>
                </div>
                <div class="card-body">
                    <div class="alert alert-warning">
                        <strong><i class="fas fa-info-circle me-2"></i>Format File:</strong>
                        <ul class="mb-0 mt-2">
                            <li>File harus berformat <strong>CSV</strong> atau <strong>TXT</strong> (untuk Excel, simpan sebagai CSV dulu)</li>
                            <li><strong>Mode Import Sisa Saldo (Recommended):</strong> Format: <strong>NIK, Tahun, Sisa Saldo, Keterangan (opsional)</strong></li>
                            <li>Contoh: <code>00012345,2025,10</code> (sisa saldo 10 hari)</li>
                            <li>Sistem akan otomatis menghitung saldo tahun lalu dan tahun ini berdasarkan penggunaan cuti</li>
                            <li><strong>Mode Import Detail:</strong> Format: <strong>NIK, Tahun, Saldo Tahun Lalu, Saldo Tahun Ini, Keterangan (opsional)</strong></li>
                            <li>Contoh: <code>00012345,2025,5,12,Import dari Excel</code></li>
                            <li>Baris pertama bisa dijadikan header (akan di-skip jika opsi "Skip Header" dicentang)</li>
                            <li><a href="{{ asset('TEMPLATE_SALDO_CUTI.csv') }}" download class="btn btn-sm btn-outline-primary mt-2">
                    ...
                    <form id="uploadForm" enctype="multipart/form-data">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-2">
                                <label for="import_mode" class="form-label">Mode Import</label>
                                <select class="form-select" id="import_mode" name="import_mode">
                                    <option value="sisa_saldo" selected>Sisa Saldo</option>
                                    <option value="detail">Detail (Tahun Lalu & Tahun Ini)</option>
                                </select>
                            </div>
                            <div class="col-md-2">
                                <label for="skip_header" class="form-label">Skip Header</label>
                                <div class="form-check form-switch mt-2">
                                    <input class="form-check-input" type="checkbox" id="skip_header" name="skip_header" checked>
                                    <label class="form-check-label" for="skip_header">Ya</label>
                                </div>
                            </div>
                            <div class="col-md-2">
                            ...
                            <div class="col-md-3 d-flex align-items-end">
                                <button type="submit" class="btn btn-success w-100 shadow-sm" id="btnUpload">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (uploadForm) {
            uploadForm.addEventListener('submit', function(e) {
                ...
                formData.append('skip_header', document.getElementById('skip_header').checked ? '1' : '0');
                formData.append('separator', document.getElementById('separator').value);

                // Disable button dan show loading
                btnUpload.disabled = true;
                const originalText = btnUpload.innerHTML;
                btnUpload.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i>Memproses...';

                // Show result area
                uploadResult.style.display = 'block';
                uploadResult.innerHTML = '<div class="alert alert-info"><i class="fas fa-spinner fa-spin me-2"></i>Sedang memproses file...</div>';

                fetch('{{ route("saldo-cuti.import") }}', {
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            ...
                            uploadResult.innerHTML = '<div class="alert alert-success"><strong><i class="fas fa-check-circle me-2"></i>Berhasil!</strong><br>' + message + '</div>';

                            // Reload halaman setelah 2 detik untuk refresh data
                            setTimeout(() => {
                                window.location.reload();
                            }, 2000);
                        } else {
                            const message = String(data.message || 'Terjadi kesalahan').replace(/\n/g, '<br>');
                            uploadResult.innerHTML = '<div class="alert alert-danger"><strong><i class="fas fa-exclamation-circle me-2"></i>Gagal!</strong><br>' + message + '</div>';
                        }
                    })
                    .catch(error => {
                        btnUpload.disabled = false;
                        btnUpload.innerHTML = originalText;
                        uploadResult.innerHTML = '<div class="alert alert-danger"><strong><i class="fas fa-exclamation-circle me-2"></i>Error!</strong><br>' + error.message + '</div>';
